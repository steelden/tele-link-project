...
#ifdef OS_WIN_STORE
		return appData.absolutePath() + u"/TeleLink UWP/"_q;
#else // OS_WIN_STORE
...
void psDoFixPrevious() {
	try {
		...
		if (SUCCEEDED(userDesktopRes)) {
			userDesktopLnk = QString::fromWCharArray(userDesktopFolder) + "\\TeleLink.lnk";
		}
		if (SUCCEEDED(commonDesktopRes)) {
			commonDesktopLnk = QString::fromWCharArray(commonDesktopFolder) + "\\TeleLink.lnk";
		}
...
void AutostartToggle(bool enabled, Fn<void(bool)> done) {
	...
	const auto success = ManageAppLink(
		...
		FOLDERID_Startup,
		L"-autostart",
		L"TeleLink autorun link.\n"
		"You can disable autorun in TeleLink settings.");
	if (done) {
...
void psSendToMenu(bool send, bool silent) {
	ManageAppLink(
		...
		FOLDERID_SendTo,
		L"--",
		L"TeleLink send to link.\n"
		"You can disable send to menu item in TeleLink settings.");
}
